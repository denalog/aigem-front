...
function makePatients(count = 20): Patient[] {
  ...
  while (arr.length < count) {
    const ward = (Math.random() < 0.5 ? "A" : "B") as WardId;
    const floors = WARDS.find((w) => w.id === ward)!.floors;
    const floor = floors[Math.floor(Math.random() * floors.length)];
    const roomNo = 1 + Math.floor(Math.random() * 5); // 1~5호
    const key = `${ward}-${floor}-${roomNo}`;
    if (used.has(key)) continue;
    used.add(key);

    const code = `${ward}-${floor}0${roomNo}호`;
    arr.push({
      id: id++,
      ward,
      floor,
      ...
      last: TIMES[Math.floor(Math.random() * TIMES.length)],
    });
  }
  // 정렬: 병동 > 층 > 호
  return arr.sort(
    (a, b) =>
      a.ward.localeCompare(b.ward) || a.floor - b.floor || a.roomNo - b.roomNo
  );
}

const ALL = makePatients(20);
...
export default function DoctorDashboard() {
  ...
  const filtered = useMemo(() => {
    ...
    if (floorFilter !== "전체")
      arr = arr.filter((p) => p.floor === floorFilter);
    if (statusFilter !== "전체")
    ...
    return arr;
  }, [wardFilter, floorFilter, statusFilter, q]);
  ...
  // 선택된 병동에서 선택 가능한 층 목록
  const availableFloors = useMemo(() => {
    if (wardFilter === "전체") {
      // 현재 결과 집합에서 존재하는 층만 추출
      return Array.from(new Set(filtered.map((p) => p.floor))).sort(
        (a, b) => a - b
      );
    }
    ...
    <main className="page">
      <div className="inner">
        ...
        <div className="head">
          ...
          <div className="tools">
            <div className="statusChips" role="tablist" aria-label="상태 필터">
              ...
              <button
                ...
                위험
                {counts.danger > 0 && (
                  <span className="badge">{counts.danger}</span>
                )}
              </button>
              ...
              <button
                ...
                주의
                {counts.warn > 0 && (
                  <span className="badge">{counts.warn}</span>
                )}
              </button>
          ...
          <div className="views">
            <div className="wardChips" role="tablist" aria-label="병동 선택">
              <button
                className={`wbtn ${wardFilter === "전체" ? "on" : ""}`}
                onClick={() => {
                  setWardFilter("전체");
                  setFloorFilter("전체");
                }}>
        ...
        <div className="sections">
          {(wardFilter === "전체" ? WARDS.map((w) => w.id) : [wardFilter]).map(
            (ward) => {
              ...
              // 선택된 층만 표시
              const keys = Array.from(floorsMap.keys())
                .sort((a, b) => a - b)
      ...
      <style jsx>{`
        ...
        .sbtn.on {
          border-color: #4a86ff;
          background: #fff;
          box-shadow: 0 6px 14px rgba(21, 44, 84, 0.12);
          color: #193a8a;
        }
        .sbtn.ok {
          background: #ecfdf5;
        ...
        .search input {
          ...
          background: #fff;
          box-shadow: 0 2px 10px rgba(21, 44, 84, 0.06);
          font-weight: 700;
          color: #0b1b33;
        }
        .alertBtn {
        ...
        .fbtn {
          ...
          font-weight: 800;
          color: #0b1b33;
          box-shadow: 0 2px 8px rgba(21, 44, 84, 0.06);
        }
        .wbtn.on,
        .fbtn.on {
          background: #2f6fe4;
        ...
        .card {
          ...
          box-shadow: 0 6px 14px rgba(21, 44, 84, 0.08);
          padding: 12px;
          transition: transform 0.12s ease, box-shadow 0.12s ease;
        }
        .card:hover {
          transform: translateY(-2px);
          box-shadow: 0 16px 24px rgba(21, 44, 84, 0.12);
        }
