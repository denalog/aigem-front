...
const AlertHistory = dynamic(
  () =>
    import("../../components/alerthistory/nurse").catch(() => () => (
...
export default function NurseDashboard() {
  ...
  // 로그인 간호사 표시 (세션에 없으면 기본값)
  const [nurseName, setNurseName] = useState("박소연");
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("aigem-dev-mock");
      if (raw) {
        const s = JSON.parse(raw);
        if (s?.role === "nurse" && s?.name) setNurseName(s.name);
      }
    } catch {}
  }, []);
  ...
  // ✅ 간호사 상세 진입: 항상 김복순 고정
  const goDetail = () => router.push("/dashboard/nurse/patient/p001");
    ...
    <main className="page">
      <div className="inner">
        ...
        <div className="head">
          <div className="title">
            <h1>실시간 모니터링</h1>
            <span className="who" title={`간호사: ${nurseName}`} aria-label={`담당 간호사: ${nurseName}`}>
              👩‍⚕️ {nurseName} <span className="role">간호사</span>
            </span>
            <span className="live">
                      ...
                      >
                        <div className="row1">
                          ...
                          <div className="meta">
                            ...
                            {/* ✅ 담당의(작은 글씨) */}
                            <div className="mini">담당의: {demoDoctor.name}</div>
                          </div>
      ...
      <style jsx>{`
        ...
        .mini{margin-top:2px;font-size:11px;color:#7a8fbf;font-weight:800;}
        ...
        .panel{width:min(92vw,760px);max-height:80vh;overflow:auto;background:#fff;border-radius:16px;border:1px solid #e1e9ff;box-shadow:0 20px 40px rgba(21,44,84,.18);}
