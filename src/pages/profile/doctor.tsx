// src/pages/profile/doctor.tsx
import { useEffect, useMemo, useState } from "react";
import { useRouter } from "next/router";

type Role = "doctor";

function roleLabel(_: Role) {
  return "의사";
}
...
export default function DoctorProfilePage() {
  ...
  const DEFAULT = {
    name: "김문수",
    email: "munsu.kim@aigem.dev",
    license: "MD-2012-04567",
    hospital: "AIGEM 요양병원",
    department: "내과",
    ward: "3병동",
  };
  ...
  useEffect(() => {
    ...
    const lic= localStorage.getItem("doctorLicense") || DEFAULT.license;
  ...
  const initial = useMemo(
    () => (name?.trim() ? name.trim().charAt(0) : "의").toUpperCase(),
    [name]
  );
  ...
  return (
    <main className="wrap">
      ...
      <section className="head">
        <h1>내 프로필</h1>
        <p className="muted">의사 계정의 기본 정보와 연락처를 확인합니다.</p>
      </section>
        ...
        <article className="card hero">
          ...
          <div className="info">
            ...
            <div className="line2">
              ...
              <span className="email">{email || "의사 이메일"}</span>
            </div>
